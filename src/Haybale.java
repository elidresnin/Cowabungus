...
// Written by: Mr. Swope
// Date: 1/27/2020
// Description: This class implements a Haybale.  This Item will be drawn onto a graphics panel. 
// 
...
public class Haybale{
	...
	public Haybale(){
		this(500, 300, "images/forest/objects/Tree_2.png", 2);
	}
	...
	public Haybale(int x_coordinate, int y_coordinate, String imageString){
	...
	public Haybale(int x_coordinate, int y_coordinate, String imageString, int imageScale){
	...
	public void changeScale(int imageScale) {
		if(image.getIconWidth(
				) > imageScale && image.getIconHeight() > imageScale) {
			Image scaled = image.getImage().getScaledInstance(image.getIconWidth() / imageScale, 
					image.getIconHeight() / imageScale, Image.SCALE_SMOOTH);
	...
	public Rectangle getBounds(){
		return new Rectangle(x_coordinate+50, y_coordinate, image.getIconWidth(), 
				image.getIconHeight());
	}
	...
	// description: This method will return true if the Item collides with the parameter Item.
	// return: boolean - true if the two items intersect or collide with each other, otherwise false. 
	public boolean collision(Carrot otherItem) {
